...
//			localServiceName = serviceName;
     ...
     * @member _createDataAndChangeSet
     */ 
    ...
    this._fireAfterSaveChanges = function (success, request) {
        ...
        // Now that all event handling triggers have run, can delete the prods properties
        var clearErrorString =  this.autoApplyChanges ? true : false;
    
        // This will be set if submit operation was performed
        if (request.jsrecords) {
            for (var idx = 0; idx < request.jsrecords.length; idx++) {
                var jsrecord = request.jsrecords[idx];
                this._deleteProdsProperties(jsrecord.data, clearErrorString);
            }
        }
        else if (request.batch && request.batch.operations) {
            for (var i = 0; i < request.batch.operations.length; i++) {
				var operRequest = request.batch.operations[i];
                this._deleteProdsProperties(operRequest.jsrecord.data, clearErrorString);
            }
        }
    }
...
//        var hash = base64_encode(tok);
     ...
     * @member handleJSONLoginResponse
     */
